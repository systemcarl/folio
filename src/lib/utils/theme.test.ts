...
import { defaultTheme, getFonts, getSection, loadThemes } from './theme';
...
vi.mock('$lib/utils/logs', () => ({ log : vi.fn() }));
...
describe('loadThemes', () => {
  ...
  it('returns default theme if fetch fails', async () => {
    ...
    const result = await loadThemes({ fetch : fetchMock });
    expect(result).toEqual(defaultThemes);
  });

  it('returns default theme if fetch not ok', async () => {
    fetchMock.mockResolvedValue({ ok : false, json : async () => themes });
    const result = await loadThemes({ fetch : fetchMock });
    expect(result).toEqual(defaultThemes);
  });
...
describe('getSection background', () => {
  ...
  it.each([
    { mode : 'cover' },
    { mode : 'tile' },
  ])('returns valid background image mode', ( { mode } ) => {
    const theme = {
      ...testTheme,
      backgrounds : {
        ...testTheme.backgrounds,
